...
export const CONFIG_ERROR_CODES = {
  ...
  /**
   * A network declaration disagrees with STELLAR_NETWORK — passphrase (#1133),
   * Horizon URL (#1134), or Soroban RPC URL (#1135). Raised by
   * packages/shared/src/networkConsistency.ts, which every service boot path
   * calls; see docs/env-validation.md#network-consistency-gate.
   */
...
export function loadBaseConfig(env: Env = processEnv): BaseConfig {
  ...
  // Network consistency gate (#1133, #1134, #1135): STELLAR_NETWORK,
  // SOROBAN_NETWORK_PASSPHRASE, STELLAR_HORIZON_URL(S) and STELLAR_RPC_URL(S)
  // must all describe the same chain. Fail closed here so a half-rotated
  // deployment aborts at boot instead of at the first money-path call.
  assertStellarNetworkConsistency(env);
  warnUnverifiableStellarEndpoints(env, nodeEnv);
