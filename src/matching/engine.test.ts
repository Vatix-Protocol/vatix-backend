...
describe("matchOrder", () => {
  ...
  describe("Basic Matching", () => {
    it("should match buy order with sell order at same price", () => {
      ...
      orderBook.addOrder(createBookOrder("sell-1", "ask", 0.5, 100, 1000));
    ...
    it("should match sell order with buy order at same price", () => {
      ...
      orderBook.addOrder(createBookOrder("buy-1", "bid", 0.5, 100, 1000));

      // Create a sell order at the same price
      const sellOrder = createMatchingOrder("sell-1", "SELL", 0.5, 100);
    ...
    it("should match buy order when ask price is lower than bid", () => {
      orderBook.addOrder(createBookOrder("sell-1", "ask", 0.4, 100, 1000));
      ...
      expect(result.trades.length).toBe(1);
      expect(result.trades[0].price).toBe(0.4); // Maker's price
      expect(result.trades[0].quantity).toBe(100);
      expect(result.remainingOrder).toBeNull();
    });

    it("should match sell order when bid price is higher than ask", () => {
      orderBook.addOrder(createBookOrder("buy-1", "bid", 0.6, 100, 1000));
      ...
      expect(result.trades.length).toBe(1);
      expect(result.trades[0].price).toBe(0.6); // Maker's price
      expect(result.trades[0].quantity).toBe(100);
      expect(result.remainingOrder).toBeNull();
    });
  });
  ...
  describe("Partial Fills", () => {
    it("should partially fill large buy order against smaller sell", () => {
      orderBook.addOrder(createBookOrder("sell-1", "ask", 0.5, 50, 1000));
    ...
    it("should fill buy order against multiple sell orders", () => {
      ...
      orderBook.addOrder(createBookOrder("sell-3", "ask", 0.5, 50, 3000));
      ...
      expect(result.trades[2].price).toBe(0.5);
    ...
    it("should fill sell order against multiple buy orders", () => {
      ...
      orderBook.addOrder(createBookOrder("buy-3", "bid", 0.5, 50, 3000));
  ...
  describe("Price-Time Priority", () => {
    it("should match with best price first (lowest ask for buy)", () => {
      orderBook.addOrder(createBookOrder("sell-high", "ask", 0.6, 50, 1000));
      orderBook.addOrder(createBookOrder("sell-low", "ask", 0.4, 50, 2000));
      orderBook.addOrder(createBookOrder("sell-mid", "ask", 0.5, 50, 3000));

      const buyOrder = createMatchingOrder("buy-1", "BUY", 0.6, 50);

      const result = matchOrder(buyOrder, orderBook);

      expect(result.trades.length).toBe(1);
      expect(result.trades[0].sellOrderId).toBe("sell-low");
      expect(result.trades[0].price).toBe(0.4);
    });

    it("should match with best price first (highest bid for sell)", () => {
      orderBook.addOrder(createBookOrder("buy-low", "bid", 0.4, 50, 1000));
      orderBook.addOrder(createBookOrder("buy-high", "bid", 0.6, 50, 2000));
      orderBook.addOrder(createBookOrder("buy-mid", "bid", 0.5, 50, 3000));

      const sellOrder = createMatchingOrder("sell-1", "SELL", 0.4, 50);
    ...
    it("should respect time priority at same price level", () => {
      ...
      orderBook.addOrder(createBookOrder("sell-first", "ask", 0.5, 30, 1000));
      orderBook.addOrder(createBookOrder("sell-second", "ask", 0.5, 30, 2000));
      orderBook.addOrder(createBookOrder("sell-third", "ask", 0.5, 30, 3000));

      const buyOrder = createMatchingOrder("buy-1", "BUY", 0.5, 60);
  ...
  describe("No Match Scenarios", () => {
    it("should return no trades when no matching orders exist", () => {
      ...
      orderBook.addOrder(createBookOrder("sell-1", "ask", 0.6, 100, 1000));

      const buyOrder = createMatchingOrder("buy-1", "BUY", 0.4, 100);
    ...
    it("should return no trades when prices do not overlap", () => {
      ...
      orderBook.addOrder(createBookOrder("sell-1", "ask", 0.7, 100, 1000));
    ...
    it("should not match buy against other bids", () => {
      ...
      orderBook.addOrder(createBookOrder("bid-1", "bid", 0.5, 100, 1000));
    ...
    it("should not match sell against other asks", () => {
      ...
      orderBook.addOrder(createBookOrder("ask-1", "ask", 0.5, 100, 1000));
  ...
  describe("Edge Cases", () => {
    ...
    it("should return null remainingOrder when fully filled", () => {
      orderBook.addOrder(createBookOrder("sell-1", "ask", 0.5, 200, 1000));
    ...
    it("should stop matching when price limit reached", () => {
      orderBook.addOrder(createBookOrder("sell-1", "ask", 0.4, 50, 1000));
      orderBook.addOrder(createBookOrder("sell-2", "ask", 0.45, 50, 2000));
      orderBook.addOrder(createBookOrder("sell-3", "ask", 0.6, 50, 3000)); // Too expensive

      const buyOrder = createMatchingOrder("buy-1", "BUY", 0.5, 150);
  ...
  describe("Order Book Integrity", () => {
    ...
    it("should maintain order book consistency after multiple matches", () => {
      ...
      orderBook.addOrder(createBookOrder("sell-3", "ask", 0.6, 100, 3000));

      // First match: consumes sell-1 fully
      matchOrder(createMatchingOrder("buy-1", "BUY", 0.5, 100), orderBook);
  ...
  describe("Trade Record Correctness", () => {
    it("should set correct buyer/seller based on order side (buy order)", () => {
      const sellerAddress =
        "GSELLER234567890123456789012345678901234567890123456";
      orderBook.addOrder(
        createBookOrder("sell-1", "ask", 0.5, 100, 1000, sellerAddress)
      );

      const buyerAddress =
        "GBUYER1234567890123456789012345678901234567890123456";
      const buyOrder = createMatchingOrder(
        "buy-1",
        "BUY",
        0.5,
        100,
    ...
    it("should set correct buyer/seller based on order side (sell order)", () => {
      const buyerAddress =
        "GBUYER1234567890123456789012345678901234567890123456";
      orderBook.addOrder(
        createBookOrder("buy-1", "bid", 0.5, 100, 1000, buyerAddress)
      );

      const sellerAddress =
        "GSELLER234567890123456789012345678901234567890123456";
      const sellOrder = createMatchingOrder(
        "sell-1",
        "SELL",
        0.5,
        100,
    ...
    it("should use maker price as execution price", () => {
      ...
      const buyOrder = createMatchingOrder("buy-1", "BUY", 0.6, 100);

      const result = matchOrder(buyOrder, orderBook);

      expect(result.trades[0].price).toBe(0.4); // Maker's price, not taker's
    });

    it("should generate unique trade IDs", () => {
      orderBook.addOrder(createBookOrder("sell-1", "ask", 0.45, 50, 1000));
      orderBook.addOrder(createBookOrder("sell-2", "ask", 0.5, 50, 2000));
  ...
  describe("Performance", () => {
    it("should efficiently match against 100+ orders in book", () => {
      ...
      for (let i = 0; i < 100; i++) {
        orderBook.addOrder(
          createBookOrder(`sell-${i}`, "ask", 0.01 + i * 0.009, 10, i)
        );
      }
    ...
    it("should handle large quantity matches efficiently", () => {
      // Single large order
      orderBook.addOrder(createBookOrder("sell-1", "ask", 0.5, 1000000, 1000));

      const start = performance.now();

      const buyOrder = createMatchingOrder("buy-1", "BUY", 0.5, 500000);
      const result = matchOrder(buyOrder, orderBook);
  ...
  describe("Position Deltas", () => {
    it("should calculate position deltas for buyer and seller on YES trade", () => {
      ...
      const buyerDelta = result.positionDeltas.find(
        (d) => d.userAddress === buyerAddress
      );
      const sellerDelta = result.positionDeltas.find(
        (d) => d.userAddress === sellerAddress
      );
    ...
    it("should aggregate position deltas across multiple trades", () => {
      ...
      orderBook.addOrder(
        createBookOrder("sell-1", "ask", 0.45, 50, 1000, seller1)
      );
      orderBook.addOrder(
        createBookOrder("sell-2", "ask", 0.5, 50, 2000, seller2)
      );
      ...
      const seller1Delta = result.positionDeltas.find(
        (d) => d.userAddress === seller1
      );
      expect(seller1Delta?.yesSharesDelta).toBe(-50);

      const seller2Delta = result.positionDeltas.find(
        (d) => d.userAddress === seller2
      );
    ...
    it("should handle same user trading with multiple counterparties", () => {
      ...
      orderBook.addOrder(
        createBookOrder("sell-1", "ask", 0.45, 30, 1000, seller)
      );
      orderBook.addOrder(
        createBookOrder("sell-2", "ask", 0.5, 70, 2000, seller)
      );
  ...
  describe("Atomicity", () => {
    it("should return consistent result with trades and position deltas", () => {
      orderBook.addOrder(createBookOrder("sell-1", "ask", 0.45, 30, 1000));
      orderBook.addOrder(createBookOrder("sell-2", "ask", 0.5, 70, 2000));

      const buyOrder = createMatchingOrder("buy-1", "BUY", 0.5, 100);
      const result = matchOrder(buyOrder, orderBook);

      const totalTradeQty = result.trades.reduce(
